...
let q = [];
q.push({i:mi, j:mj, flag:false, ti:-1, tj:-1, from:'', to:'', type:''});
while(q.length>0){
    let cn = q.shift();
    let ci = cn.i;
    let cj = cn.j;
    ...
    if(map[ci][cj]==='M'){
        ...
        if(checkUp){
            q.push({i:ci+up[0], j:cj+up[1], flag:cn.flag, ti:cn.ti, tj:cn.tj, from:'', to:'up'});
        }
        if(checkDown){
            q.push({i:ci+down[0], j:cj+down[1], flag:cn.flag, ti:cn.ti, tj:cn.tj, from:'', to:'down'});
        }
        if(checkLeft){
            q.push({i:ci+left[0], j:cj+left[1], flag:cn.flag, ti:cn.ti, tj:cn.tj, from:'', to:'left'});
        }
        if(checkRight){
            q.push({i:ci+right[0], j:cj+right[1], flag:cn.flag, ti:cn.ti, tj:cn.tj, from:'', to:'right'});
        }
    }
    ...
    if(map[ci][cj]==='|'){
        if(from==='up'){
            q.push({i:ci+down[0], j:cj+down[1], flag:cn.flag, ti:cn.ti, tj:cn.tj, from:from, to:'down'});
        }
        if(from==='down'){
            q.push({i:ci+up[0], j:cj+up[1], flag:cn.flag, ti:cn.ti, tj:cn.tj, from:from, to:'up'});
        }
    }
    if(map[ci][cj]==='-'){
        if(from==='left'){
            q.push({i:ci+right[0], j:cj+right[1], flag:cn.flag, ti:cn.ti, tj:cn.tj, from:from, to:'right'});
        }
        if(from==='right'){
            q.push({i:ci+left[0], j:cj+left[1], flag:cn.flag, ti:cn.ti, tj:cn.tj, from:from, to:'left'});
        }
    }
    if(map[ci][cj]==='+'){
        if(from==='up'){
